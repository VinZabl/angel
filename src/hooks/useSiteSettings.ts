  ...
  const fetchSiteSettings = async () => {
    try {
      ...
      const settings: SiteSettings = {
        site_name: data.find(s => s.id === 'site_name')?.value || 'Angel Game Credits',
        site_logo: data.find(s => s.id === 'site_logo')?.value || '/logo.png',
        site_description: data.find(s => s.id === 'site_description')?.value || 'Welcome to Angel Game Credits — Your perfect game credits destination',
